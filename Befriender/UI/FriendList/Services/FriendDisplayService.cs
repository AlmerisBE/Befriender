...
public class FriendDisplayService : IFriendDisplayService {
    ...
    public IReadOnlyList<FriendProfile> ProcessFriends(IEnumerable<FriendProfile> friends, bool showOnlineOnly, int sortColumnIndex, bool isAscending) {
        ...
        query = sortColumnIndex switch {
            ...
            4 => isAscending ? query.OrderBy(f => this.gameDataService.GetDisplayLocation(f.LocationId, f.CurrentWorldId, f.HomeWorldId, f.OnlineStateMask)).ThenBy(f => f.Name) : query.OrderByDescending(f => this.gameDataService.GetDisplayLocation(f.LocationId, f.CurrentWorldId, f.HomeWorldId, f.OnlineStateMask)).ThenBy(f => f.Name),
            _ => query
        };
